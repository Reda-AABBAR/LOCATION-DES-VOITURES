...
export default function EditCharacteristic() {
    ...
    const onSubmit = async (e) => {
      e.preventDefault();
      try {
          await axios.put(`  http://localhost:8080/caractiristiques/${id}`, companie);
    ...
    const loadCompanie =async ()=>{
      const result = await axios.get(`  http://localhost:8080/caractiristiques/${id}`);
    }
  ...
  return (
    ...
    <div  style={{ color: '#1a237e' ,backgroundColor:'#d3d3d3',height:'55px'}}><h2>Edit a Characteristic</h2></div>
    ...
    <form onSubmit={(e) => onSubmit(e)}>
      ...
      <div className="d-flex justify-content-between align-items-center">
        ...
        <Link type="submit"  className="btn btn-danger py-2 m-0" to="/characteristic">
        Cancel
        </Link>
      </div>
    </form>
