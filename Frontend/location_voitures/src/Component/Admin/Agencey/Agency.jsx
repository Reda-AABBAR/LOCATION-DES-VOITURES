...
export default function Agency() {
  ...
  const getAllCompanies=async()=>{
    try {
      const result = await axios.get("http://localhost:8080/agences");
  ...
  const supprimerCompanie =async(id)=>{
    try {
     await axios.delete(` http://localhost:8080/agences/${id}`);
     getAllCompanies();
    } catch (error) {
    console.log("erreu :"+error);
  }
...
const onSubmit = async (e) => {
    e.preventDefault();
    try {
        await axios.post(" http://localhost:8080/agences", companie);
        getAllCompanies();
    } catch (error) {
  ...
  return (
              ...
              companies.map((companie,index)=>(
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <Link className="btn  mx-2" to={`/editagency/${companie.id}`}>
    ...
    <div  style={{ color: '#1a237e' ,backgroundColor:'#d3d3d3',height:'55px'}}><h2>add a Agency</h2></div>
    ...
    <form onSubmit={(e) => onSubmit(e)}>
            ...
            <div className="mb-3">
            <label htmlFor="userName" style={{ color: 'white' }}>City </label>
                <input 
                 value={companie.ville}
                 onChange={(e) => onInputChange(e)}
                className="form-control my-input my-2" 
                type="text" 
                id="nom"
            ...
            <div className="mb-3">
            <label htmlFor="userName" style={{ color: 'white' }}>Adress </label>
                <input 
                 value={companie.adresse}
                 onChange={(e) => onInputChange(e)}
                className="form-control my-input my-2" 
                type="text" 
                id="nom"
