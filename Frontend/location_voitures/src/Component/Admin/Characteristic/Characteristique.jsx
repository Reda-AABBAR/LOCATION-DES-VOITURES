...
import { faBuilding } from '@fortawesome/free-regular-svg-icons';
 ...
 function Characteristique() {
  ...
  const getAllCompanies=async()=>{
    try {
      const result = await axios.get("http://localhost:8080/caractiristiques");
  ...
  const supprimerCompanie =async(id)=>{
    try {
     await axios.delete(` http://localhost:8080/caractiristiques/${id}`);
     getAllCompanies();
    } catch (error) {
    console.log("erreu :"+error);
  }
...
const onSubmit = async (e) => {
    e.preventDefault();
    try {
        await axios.post("http://localhost:8080/caractiristiques", companie);
        getAllCompanies();
    } catch (error) {
  ...
  return (
              ...
              companies.map((companie,index)=>(
                <tr key={index}>
                  ...
                  <td>
                    ...
                    <Link className="btn  mx-2" to={`/EditCharacteristic/${companie.id}`}>
    ...
    <div  style={{ color: '#1a237e' ,backgroundColor:'#d3d3d3',height:'55px'}}><h2>add a Characteristic</h2></div>
...
export default Characteristique;
