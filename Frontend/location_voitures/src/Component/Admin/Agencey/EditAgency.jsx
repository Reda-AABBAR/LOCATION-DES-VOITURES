...
export default function EditAgency() {
    ...
    const onSubmit = async (e) => {
      e.preventDefault();
      try {
          await axios.put(` http://localhost:8080/agences/${id}`, companie);
          navigate("/agency")
      } catch (error) {
    ...
    const loadCompanie =async ()=>{
      const result = await axios.get(` http://localhost:8080/agences/${id}`);
    }
  ...
  return (
    ...
    <div  style={{ color: '#1a237e' ,backgroundColor:'#d3d3d3',height:'55px'}}><h2>Edit a Agency</h2></div>
    ...
    <div className="mb-3">
      ...
      <div className="d-flex justify-content-between align-items-center">
        ...
        <Link type="submit"  className="btn btn-danger py-2 m-0" to="/agency">
        Cancel
        </Link>
      </div>
    </form>
